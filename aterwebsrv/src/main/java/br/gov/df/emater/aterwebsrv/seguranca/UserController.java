...
public class UserController {
	...
	public ResponseEntity<String> changePassword(@RequestBody final Usuario user) {
		final Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
		final Usuario currentUser = userRepository.findByNomeUsuario(authentication.getName());
...
//		currentUser.setPassword(pwEncoder.encode(user.getNewPassword()));
	...
	public ResponseEntity<String> grantRole(@PathVariable Usuario user, @PathVariable UserRole role) {
		if (user == null) {
