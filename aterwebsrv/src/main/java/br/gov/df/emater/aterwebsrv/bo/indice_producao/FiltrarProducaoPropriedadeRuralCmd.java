...
public class FiltrarProducaoPropriedadeRuralCmd extends _Comando {
	...
	public boolean executar(_Contexto contexto) throws Exception {
		...
		if (producaoProprietarioList != null) {
			...
			result = processaProducaoProprietario(filtro, producaoProprietarioList);
		}
	...
	private List<Object> fetchProducao(Producao producao) {
		return fetchProducao(producao, null);
	}
	...
	private List<Object> processaProducaoProprietario(IndiceProducaoCadFiltroDto filtro, List<ProducaoProprietario> lista) {
		List<Object> result = null;
		...
		for (ProducaoProprietario produtorProducaoProprietario : lista) {
			if (bemId != produtorProducaoProprietario.getBemClassificado().getId()) {
				bemClassificacaoList = utilDao.ipaBemClassificacaoDetalhes(produtorProducaoProprietario.getBemClassificado().getBemClassificacao());
				...
				bemId = produtorProducaoProprietario.getBemClassificado().getId();
			}
			...
			if (produtorProducaoProprietario.getProducaoList() != null) {
				...
				for (Producao producao : produtorProducaoProprietario.getProducaoList()) {
					String composicao = ProducaoCalculo.getComposicao(producao);
					Integer publicoAlvoId = produtorProducaoProprietario.getPublicoAlvo().getId();
				...
				result.add(fetch(filtro, produtorProducaoProprietario, calculoProdutorEsperada, calculoProdutorConfirmada));
			}
		}
		return result;
	}
