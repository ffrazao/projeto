...
angular.module(pNmModulo).controller(pNmController,
    ...
    var init = function() {
        if (!angular.isArray($scope.cadastro.registro.atividadeAssuntoList)) {
            $scope.cadastro.registro.atividadeAssuntoList = [];
        }
        $scope.atividadeAssuntoNvg = new FrzNavegadorParams($scope.cadastro.registro.atividadeAssuntoList, 4);
    };
    init();
    ...
    $scope.incluir = function() {
        ...
        $scope.cadastro.registro.atividadeAssuntoList.push({});
    };
    ...
    $scope.$watch('cadastro.registro.atividadeAssuntoList', function() {
        if (angular.isArray($scope.cadastro.registro.atividadeAssuntoList) && !angular.isArray($scope.atividadeAssuntoNvg.dados)) {
            $scope.atividadeAssuntoNvg.setDados($scope.cadastro.registro.atividadeAssuntoList);
            $scope.atividadeAssuntoNvg.botao('edicao').visivel = false;
            return;
        }
        if (!angular.isArray($scope.cadastro.registro.atividadeAssuntoList)) {
        ...
        if ($scope.cadastro.registro.atividadeAssuntoList.length !== $scope.atividadeAssuntoNvg.dados.length) {
            $scope.atividadeAssuntoNvg.setDados($scope.cadastro.registro.atividadeAssuntoList);
            $scope.atividadeAssuntoNvg.botao('edicao').visivel = false;
            return;
        }
